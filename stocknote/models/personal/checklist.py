...
class ChecklistBanlanceSheet(db.Model):
    ...
    hbzj_sx = db.Column(db.Text, default="-", comment="有没有权利受限的资金？包括外埠存款、银行汇票存款、银行本票存款、信用证存款和信用卡存款等")
    hbzj_yc = db.Column(db.Text, default="-", comment="是否存在隐藏的货币资金？有些上市公司的货币资金并非全部在货币资金科目核算，比如很多公司会购买理财（短期理财）。")
    hbzj_ly = db.Column(db.Text, default="-", comment="货币资金的来源")
    ...
    yfzk_zl = db.Column(db.Text, default="-", comment="查看预付款账龄")
    yfzk_xz = db.Column(db.Text, default="-", comment="关注预付款性质")
    yfzk_bl = db.Column(db.Text, default="-", comment="占总资产的比例是否超过20％？")
    ...
    tzxfc_jlfs = db.Column(db.Text, default="-", comment="投资性房产_计量方式是以成本模式计量还是以公允价值模式计量？")
    tzxfc_xzgyjzjl = db.Column(db.Text, default="-", comment="投资性房产_是否有新增的以公允价值计量的投资性房产？如由其他类型的房地产转为采用公允价值模式计量投资性房产或者由原先的成本模式变为公允价值模式。")
    ...
    ssgdqy_roe = db.Column(db.Text, default="-", comment="少数股东的ROE远高于上市公司股东的ROE?")
    create_time = db.Column(db.DateTime, default=datetime.now)
    update_time = db.Column(db.DateTime, default=datetime.now, onupdate=datetime.now)
